...
import { 
    ...
    detectTies,
    evaluateTieBreaker,
    applyCascadingTieBreakers
} from './utils/gridBuilder.js';
import { 
    generatePDF, 
    generateCSV,
    validateGridForExport,
    generateExportStats,
...
const GridBuilder = () => {
    ...
    const isWaveModified = (waveIndex) => {
        ...
        // Check if there are merged classes for this wave
        const waveMerged = mergedClasses.get(waveIndex);
        if (waveMerged && waveMerged.size > 0) return true;
    ...
    const resetWave = (waveIndex) => {
        if (originalGrid[waveIndex]) {
            ...
            // Clear merged classes for this wave
            setMergedClasses(prev => {
                const newMerged = new Map(prev);
                newMerged.delete(waveIndex);
                return newMerged;
            });
        }
    };
    ...
    const mergeClassWithPrevious = useCallback((waveIndex, className) => {
        const operationKey = `${waveIndex}-${className}-merge`;
        ...
        setFinalGrid(prev => {
            ...
            const otherEntries = wave.entries.filter(e => e.Class !== previousClassName && e.Class !== className);
            ...
            // Sort merged entries according to wave sort configuration
        ...
        setMergedClasses(prev => {
            const newMerged = new Map(prev);
            const waveMerged = newMerged.get(waveIndex) || new Map();
            
            // Get the class order for finding previous class
            const wave = finalGrid[waveIndex];
            const classOrder = getClassOrder(wave);
            const currentClassIndex = classOrder.indexOf(className);
            ...
            if (currentClassIndex > 0) {
                ...
                // Get existing merged groups or create new ones
                const previousGroup = waveMerged.get(previousClassName) || [previousClassName];
                const currentGroup = waveMerged.get(className) || [className];
                
                // Merge the groups
                const mergedGroup = [...new Set([...previousGroup, ...currentGroup])];
                
                // Update all classes in the merged group to point to the same group
                mergedGroup.forEach(cls => {
                    waveMerged.set(cls, mergedGroup);
                });
                
                newMerged.set(waveIndex, waveMerged);
            }
    ...
    const getMergedClassDisplay = (waveIndex, className) => {
        const waveMerged = mergedClasses.get(waveIndex);
                        ...
                        {waveConfigs.map((config, idx) => {
                            ...
                            return (
                                <Card key={idx}>
                                    ...
                                    {/* Empty positions between classes - Only show when grouped by class */}
                                    {(config.gridOrder === 'fastestFirst' || config.gridOrder === 'slowestFirst') && (
                                        <div>
                                            <Label htmlFor={`empty-classes-${idx}`}>
                                                Empty positions between classes
                                            </Label>
                                            <Input
                                                id={`empty-classes-${idx}`}
                                                type="number"
                                                min="0"
                                                max="5"
                                                value={config.emptyPositionsBetweenClasses || 0}
                                                onChange={(e) => updateWaveConfig(idx, 'emptyPositionsBetweenClasses', parseInt(e.target.value) || 0)}
                                                className="w-32"
                                            />
                                            <p className="text-sm text-gray-600 mt-1">
                                                Empty grid positions to leave between each class
                                            </p>
                                        </div>
                                    )}
                                            ...
                                            <CardContent className="p-0">
                                                {(() => {
                                                    if (wave.config.gridOrder === 'fastestFirst' || wave.config.gridOrder === 'slowestFirst') {
                                                        const classOrder = getClassOrder(wave);
                                                        const result = [];
                                                        const processedMergedGroups = new Set();
                                                        const waveMerged = mergedClasses.get(waveIdx) || new Map();
                                                        
                                                        // Calculate total number of displayed class groups
                                                        let totalDisplayedGroups = 0;
                                                        const groupsSeen = new Set();
                                                        classOrder.forEach(cls => {
                                                            const mergedGroup = waveMerged.get(cls);
                                                            const groupKey = mergedGroup ? mergedGroup.sort().join('-') : cls;
                                                            if (!groupsSeen.has(groupKey)) {
                                                                groupsSeen.add(groupKey);
                                                                totalDisplayedGroups++;
                                                            }
                                                        });
                                                        
                                                        let displayedClassIndex = 0;
                                                        classOrder.forEach((className, classIdx) => {
                                                            // Check if this class is part of a merged group that's already been processed
                                                            const mergedGroup = waveMerged.get(className);
                                                            const groupKey = mergedGroup ? mergedGroup.sort().join('-') : className;
                                                            
                                                            if (mergedGroup && processedMergedGroups.has(groupKey)) {
                                                                // Skip this class header as it's part of an already processed merged group
                                                                return;
                                                            }
                                                            ...
                                                            // Get all entries for this class or merged group
                                                            const classEntries = waveEntries.filter(entry => {
                                                                if (mergedGroup) {
                                                                    return mergedGroup.includes(entry.Class);
                                                                }
                                                            ...
                                                            result.push(
                                                                <div key={`class-header-${className}`} className="bg-blue-50 px-4 py-2 border-b flex items-center gap-3">
                                                                        ...
                                                                        <>
                                                                            <Button
                                                                                ...
                                                                                onClick={(e) => {
                                                                                    e.preventDefault();
                                                                                    e.stopPropagation();
                                                                                    mergeClassWithPrevious(waveIdx, className);
                                                                                }}
                                                                                title="Merge with previous class"
                                                                            >
                                                                                <img src={mergeIcon} alt="Merge with previous" className="h-4 w-4" />
                                                                            </Button>
                                                                            <Button
                                                                                size="icon"
                                                                                variant="ghost"
                                                                    ...
                                                                    <span className="font-semibold text-track-blue">{getMergedClassDisplay(waveIdx, className)}</span>
                                                                    {displayedClassIndex < totalDisplayedGroups - 1 && (
                                                                        <Button
                                                                            size="icon"
                                                                            variant="ghost"
                                                                            className="h-6 w-6"
                                                            ...
                                                            classEntries.forEach((entry, entryIdx) => {
                                                                ...
                                                                if (entry.isEmpty) {
                                                                    result.push(
                                                                        <div key={`empty-${originalEntryIdx}`} className="text-center py-3 text-gray-500 italic bg-gray-50 border-b">
                                                                            Empty Position
                                                                        </div>
                                                                    );
                                                                    return;
                                                        ...
                                                        return waveEntries.map((entry, entryIdx) => {
                                                            ...
                                                            if (entry.isEmpty) {
                                                                return (
                                                                    <div key={entryIdx} className="text-center py-3 text-gray-500 italic bg-gray-50 border-b">
                                                                        Empty Position
                                                                    </div>
                                                                );
                                                            }
